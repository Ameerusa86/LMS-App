...
const Categories: React.FC = () => {
    ...
    const fetchData = async () => {
      try {
        const categoriesSnapshot = await getDocs(
          collection(fs_database, "categories")
        );
  ...
  const handleAddCategory = async () => {
    if (newCategory.trim() !== "") {
      try {
        const docRef = await addDoc(collection(fs_database, "categories"), {
          name: newCategory,
        });
        setCategories((prev) => [
          ...prev,
  ...
  const handleEditCategory = async () => {
    if (editingCategory && editCategoryName.trim() !== "") {
      try {
        const categoryRef = doc(fs_database, "categories", editingCategory.id);
  ...
  const handleDeleteCategory = async (id: string) => {
    try {
      await deleteDoc(doc(fs_database, "categories", id));
