...
import { fs_database, auth } from "@/utils/firebase";
import {
  collection,
  getDocs,
  doc,
...
const Users: React.FC = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const usersSnapshot = await getDocs(collection(fs_database, "users"));
  ...
  const handleEditUser = async () => {
    if (
      ...
      try {
        const userDocRef = doc(fs_database, "users", editingUser.id);
  ...
  const handleDeleteUser = async (userId: string) => {
    try {
      await deleteDoc(doc(fs_database, "users", userId));
  ...
  const handleToggleAdmin = async (userId: string, isAdmin: boolean) => {
    try {
      const userDocRef = doc(fs_database, "users", userId);
